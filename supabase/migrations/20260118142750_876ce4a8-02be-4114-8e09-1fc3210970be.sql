-- Restore schedule_task_reminders to use correct column names (start_time, not scheduled_start)
-- This fixes the error: record "new" has no field "scheduled_start"
...
BEGIN
  ...
  IF NEW.status = 'DONE' OR NEW.completed_at IS NOT NULL THEN
    DELETE FROM scheduled_notifications WHERE task_id = NEW.id AND delivered_at IS NULL AND failed_at IS NULL;
    RETURN NEW;
  END IF;
...
CREATE TRIGGER schedule_task_reminders_trigger
  AFTER INSERT OR UPDATE OF status, completed_at, due_date, start_time, end_time, reminder_minutes
  ON tasks
  FOR EACH ROW
