-- =============================================================================
-- FIX: Preserve task_created notifications on UPDATE
-- The current trigger deletes ALL notifications when a task is updated.
-- This fix excludes 'task_created' from deletion so it gets delivered to Slack.
-- =============================================================================
...
BEGIN
  ...
  -- If task is marked done, delete all pending reminders
  ...
  -- Only process if task has start_time or due_date
  IF NEW.start_time IS NOT NULL OR NEW.due_date IS NOT NULL THEN
    -- FIXED: Only delete REMINDER notifications, preserve task_created
    -- This ensures Slack notification for task_created gets delivered
    DELETE FROM scheduled_notifications 
    WHERE task_id = NEW.id 
  ...
  -- Create task_created notification only on INSERT
