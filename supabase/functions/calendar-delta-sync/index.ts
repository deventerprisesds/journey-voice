...
async function syncOutlookDelta(
      ...
      if (existingEvents) {
        ...
        if (orphanIds.length > 0) {
          ...
          if (!orphanError) {
            ...
            console.log(`[calendar-delta-sync] Cleaned up ${orphanIds.length} orphan Outlook events`);
          }
        }
      }
    }
  ...
  if (purgedData && purgedData.length > 0) {
    ...
    console.log(`[calendar-delta-sync] Purged ${purgedData.length} past Outlook events older than 48h`);
  }
