      ...
      try {
        ...
        for (let dayOffset = 0; dayOffset < totalDays; dayOffset++) {
          ...
          for (const task of dedupedCandidates) {
            ...
            // MERGE UNION 2026-09-13 (trait model + caveats model both guard this branch).
            // `baseWindowCount`, NOT `preferredWindows.length`: the latter is post-caveat, so a
            // caveat that narrowed the windows would make a constrained task look unconstrained
            // and aggregate-fit would place it outside its config-allowed windows — exactly what
            // "placement is CONFIG-AUTHORITATIVE" forbids. Both guards are required, so this is
            // strictly MORE conservative than either side alone, which is the safe direction here.
            if (!assigned && !isAssignment && !windowConstrained && !keywordOverride && baseWindowCount === activeWindowNames.length) {
            ...
            if (assigned) {
              ...
              dayPlacements.push({
                ...
                // MERGE UNION: `keywordOverride` from main's restructured object; `trait` and
                // `caveatApplied` are each side's own new field and both are kept.
              ...
              // MERGE 2026-09-13: `collectOverflow` exists ONLY on this branch and is BEHAVIOUR,
              // not logging — it feeds the value-aware overflow nudge. main's side of this hunk
              // rewrote the console output around it, so taking main wholesale would have deleted
              // the nudge silently, with a green build and no failing test.
