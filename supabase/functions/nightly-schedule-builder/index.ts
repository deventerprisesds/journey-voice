...
async function deleteAppOriginatedEvents(
  ...
  // Look up which of these have app-originated events
  const taskIds = tasksWithEvents.map(t => t.id);
  ...
  // Get WRITE connection for this user
  const { data: writeConns } = await supabase
  ...
  if (!writeConns || writeConns.length === 0) {
    console.warn(`  ⚠️ No WRITE calendar connection — skipping external event deletion`);
    // Still clean up DB references
    await supabase.from('external_calendar_events').delete().in('source_task_id', taskIds);
    return;
  }

  // Try to delete from external calendar via the calendar-integration-manager
  const supabaseUrl = Deno.env.get('SUPABASE_URL')!;
  const supabaseServiceKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!;

  for (const evt of appEvents) {
    try {
      const response = await fetch(`${supabaseUrl}/functions/v1/calendar-integration-manager`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
